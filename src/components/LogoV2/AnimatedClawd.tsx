...
const IDLE_ANIMATION: readonly Frame[] = [...hold('look-right', 0, 6), ...hold('default', 0, 10), ...hold('look-left', 0, 6), ...hold('default', 0, 10), ...hold('arms-up', 0, 4), ...hold('default', 0, 12)];
const FRAME_MS = 60;
...
function useClawdAnimation(): {
  ...
  // Animation disabled — always return static default pose
  const reducedMotion = true;
  const [frameIndex, setFrameIndex] = useState(-1);
  const [isClickAnimation, setIsClickAnimation] = useState(false);
  const sequenceRef = useRef<readonly Frame[]>(IDLE_ANIMATION);
  const onClick = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (frameIndex >= sequence.length - 1) {
        if (isClickAnimation) {
          sequenceRef.current = IDLE_ANIMATION;
